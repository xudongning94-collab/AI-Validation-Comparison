...
def main() -> int:
    ...
    for summary in result.pair_summaries:
        print(
            ...
            f"high={summary.high_pairs} medium={summary.medium_pairs} "
            f"max={summary.max_similarity:.3f} "
            f"repeat_a={summary.document_a.repeated_rate_high:.2%} "
            f"repeat_b={summary.document_b.repeated_rate_high:.2%}"
        )
