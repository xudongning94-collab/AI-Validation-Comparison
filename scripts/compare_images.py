...
def main() -> int:
    ...
    parser.add_argument("--thresholds", default=str(ROOT / "configs" / "thresholds.yaml"), help="图片相似度阈值配置")
    ...
    for summary in result.pair_summaries:
        print("PAIR " f"{summary.document_a_filename} <-> {summary.document_b_filename} | " f"exact={summary.exact_pairs} high_similar={summary.high_similar_pairs} " f"max={summary.max_similarity:.3f} " f"repeat_a={summary.document_a.repeated_rate:.2%} " f"repeat_b={summary.document_b.repeated_rate:.2%}")
